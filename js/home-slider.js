/*
  Copyright © 2016 Jay Wilson / ImageSliderMaker.com - All Rights Reserved
*/
...
(function() {
  ...
  ISMQuery.prototype.fadeOut = function(duration, callback) {
    ISMQuery.crossFade(this.el, null, duration, callback);
  };

  ISMQuery.prototype.fadeIn = function(duration, callback) {
    ISMQuery.crossFade(null, this.el, duration, callback);
  };

  ISMQuery.prototype.fadeInto = function(other_el, duration, callback) {
    ISMQuery.crossFade(this.el, other_el, duration, callback);
  };

  ISMQuery.crossFade = function(el_fade_out, el_fade_in, duration, callback) {
    el_fade_out = el_fade_out || {style:{}};
    el_fade_in = el_fade_in || {style:{}};
    duration = duration || 400;
    var opacity_fade_out = 1, opacity_fade_in = 0;
    el_fade_out.style.opacity = opacity_fade_out;
    el_fade_in.style.opacity = opacity_fade_in;
    el_fade_out.style.filter = '';
    el_fade_in.style.filter = '';
    var last = +new Date();
    var delta;
    var tick = function() {
      delta = (new Date() - last) / duration;
      opacity_fade_out -= delta;
      opacity_fade_in += delta
      el_fade_out.style.opacity = opacity_fade_out;
      el_fade_in.style.opacity = opacity_fade_in;
      el_fade_out.style.filter = 'alpha(opacity=' + (100 * opacity_fade_out)|0 + ')';
      el_fade_in.style.filter = 'alpha(opacity=' + (100 * opacity_fade_in)|0 + ')';
      last = +new Date();
      if(opacity_fade_out > 0)
      {
        (window.requestAnimationFrame && requestAnimationFrame(tick)) || setTimeout(tick, 16);
      }
      else
      {
        el_fade_out.style.opacity = 0;
        el_fade_in.style.opacity = 1;
        el_fade_out.style.filter = '';
        el_fade_in.style.filter = '';
        if(callback)
        {
  ...
  function ISMSlider(element_id, options) {
    ...
    function init_config(given_opts) {
      ...
      // <div class="home-slider" data-transition-type="" data-play-type="" data-pause="" data-interval="" data-image-fx="" data-buttons="" data-radios="">
    ...
    function init_buttons() {
      ...
      //console.log("init_buttons() opts.buttons(" + opts.buttons + ")(" + !!opts.buttons + ")");
    ...
    function fade(current_slide_index, new_slide_index, zoom, callback) {
      ...
      var el_to_fade_in = ol_clone.get(0);
  ...
  iq.ready(function() {
    ...
    {
      var slider_divs = iq(".home-slider");
      for(var i = 0; i < slider_divs.length; i++)
      {
        var slider_div = slider_divs.get(i);
        slider_div.id = slider_div.id || "home-slider-" + i;
